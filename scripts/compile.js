...
console.log('====================================================');
console.log('  Midnight Compact ZK-Circuit Compiler Pipeline');
...
// Check if native compact is indeed the Midnight compact compiler (not Windows NTFS compact.exe)
try {
  const test = spawnSync('compact', ['--version'], { encoding: 'utf-8' });
  if (test.status === 0 && test.stdout.toLowerCase().includes('compact')) {
    // Make sure it's not the Windows NTFS tool
  ...
  // Not installed in Windows PATH
}

if (!compiled && process.platform === 'win32') {
  console.log('Invoking Midnight Compact Compiler via WSL toolchain...');
...
if (compiled) {
  ...
  if (fs.existsSync(zkirDir)) {
    ...
    fs.readdirSync(zkirDir).forEach((f) => {
      ...
      console.log(`  - ${f.padEnd(30)} (${sz} bytes)`);
    });
  }
